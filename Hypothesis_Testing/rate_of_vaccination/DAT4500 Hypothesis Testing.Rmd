...
```{r}
covid19_vaccinations_state_prop_new %>% 
  select(State_Territory, Percent_total_pop_with_completed_primary_series) %>% 
  arrange(Percent_total_pop_with_completed_primary_series) %>% 
  head(10)

44
91

my_cutpoints <- c(40, 50, 65, 80, 90, 100)
my_labels <- c("40 - 50", "50 - 65", "65 - 80", "80 - 90", "100 +")
...
```{r}
covid19_vaccinations_state_prop_new %>% 
  select(State_Territory, Doses_admin_by_jurisdiction_per_100K_pop) %>% 
  arrange(Doses_admin_by_jurisdiction_per_100K_pop) %>% 
  head(10)
...
```{r}
covid19_vaccinations_state_prop_new %>% 
  select(State_Territory, Percent_total_pop_with_at_least_one_dose) %>% 
  arrange(desc(Percent_total_pop_with_at_least_one_dose)) %>% 
  head(10)
